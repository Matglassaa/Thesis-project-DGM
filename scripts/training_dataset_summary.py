"""
This script summarizes and visualizes the datasets generated by FLUMY and the eventual realization generated by the GAN. 
It includes functions to plot orthogonal slices of multiple realizations, analyze and visualize the overall facies distribution, and calculate cell-wise entropy across multiple samples to assess variability.
The script also includes an optional 3D visualization using PyVista for a single random sample.

Example usage on a Linux cluster:
    nohup python -u training_dataset_summary.py --data_dir /path/to/dataset --output_dir /path/to/save/plots --num_files 10 > summary.log 2>&1 &
"""
...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='outputs/10000_training_samples/RUN_10000_samples_128xy_dataset_50_epochs_bs_32_corrected_nz/realizations', help='Path to the .npz dataset directory')
    parser.add_argument('--output_dir', type=str, default='outputs/10000_training_samples/RUN_10000_samples_128xy_dataset_50_epochs_bs_32_corrected_nz', help='Path to save plots (default: data_dir/plots)')
    parser.add_argument('--num_files', type=int, default=990, help='Number of files to sample for slice and entropy plots')
...
def load_files(file):
    """
    Loads the facies array and maps GAN labels [-1, 0, 1] 
    back to representative facies [1, 4, 8].
    """
...
def plot_facies_distribution(data_dir, output_dir, all_files):
    ...
    for i, file in enumerate(all_files):
        ...
        file_counts = np.bincount(data.ravel().round().astype(int), minlength=14)
