...
class FaciesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_one_hot:
            ...
            max_class_val = self.num_classes - 1
            processed_data = (tensor_data.float() / max_class_val) * 2.0 - 1.0
        ...
        if is_inspecting:
            ...
            print(f"Tesor looks like: {processed_data}")

        return {'data': processed_data}


if __name__ == "__main__":
    # --- CHANGED FILENAME HERE to bypass the corrupted folder ---
    TEST_NPY_PATH = "data/test_outputs_lower_plain_delta_nz_32/sample_1_facies_TEST.npy"
    
    # Extract directory context safely
    dir_path = os.path.dirname(TEST_NPY_PATH)
    if dir_path:
        os.makedirs(dir_path, exist_ok=True)
    
    # 1. Create a quick dummy .npy file if it doesn't exist
    if not os.path.exists(TEST_NPY_PATH):
        # Creates a 32x128x128 array containing dummy facies values
        dummy_data = np.random.choice([1, 5, 12], size=(32, 128, 128)).astype(np.uint8)
