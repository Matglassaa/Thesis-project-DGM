...
def parse_args():
    ...
    parser.add_argument('--csv_path', type=str, default='outputs/post_training/10000_training_samples/RUN_10000_samples_128xy_dataset_50_epochs_bs_32_equal_lr/fluvgan_1_training_1_architecture_4_dcgan_no_one_hot_1_history.csv', help='Path to the history CSV for plotting losses')
    parser.add_argument('--ckpt_path', type=str, default='outputs/post_training/10000_training_samples/RUN_10000_samples_50_epochs_bs_64_val_size_015_one_hot_tanh_equal_lr_penalty/architecture_4_dcgan_samples_one_hot_all_epochs_50_bs_64_run_1.pt', help='Path to the model checkpoint')
    parser.add_argument('--output_dir', type=str, default='outputs/post_training/10000_training_samples/RUN_10000_samples_50_epochs_bs_64_val_size_015_one_hot_tanh_equal_lr_penalty/realizations', help='Output folder')
    parser.add_argument('--num_reals', type=int, default=100, help='Number of realizations to generate')
    
    # ADDED: Dynamic configuration to match training checkpoints
    parser.add_argument('--nc', type=int, default=9, help='Number of output channels / facies classes (match training!)')
    parser.add_argument('--nl', type=int, nargs=3, default=[3, 5, 5], help='Base latent grid shape dimensions (e.g., 3 5 5 or 4 4 4)')
    parser.add_argument('--use_double_conv', action='store_true', help='Set if your trained generator used double convolutions')
...
def generate_realizations(ckpt_path, output_dir, nc, nl, use_double_conv, num_realizations=100):
    ...
    print(f"Building Generator with nc={nc}, nl={nl}, use_double_conv={use_double_conv}...")
    gen_layer = resnet.DeepGenerator3d(
        nz=nz, ngf=ngf, nc=nc, nl=tuple(nl), max_factor=max_factor, residual_weight=1.,
        ...
        last_activation=last_activation, use_double_conv=use_double_conv, use_double_resblocks=False,
    ...
    # Strip out any potential "module." prefix added by DataParallel during training
    cleaned_state = {}
    for k, v in gen_state.items():
        name = k.replace("module.", "") if k.startswith("module.") else k
        cleaned_state[name] = v

    # CHANGED: Enforce strict=True so any real mismatch throws an informative error
    try:
        gen_layer.load_state_dict(cleaned_state, strict=True)
        print("Success! Weights are perfectly aligned.")
    except RuntimeError as e:
        print("\n[CRITICAL ERROR] Architectural mismatch between script options and checkpoint weights!")
        print(f"Details: {e}")
        raise
...
def main():
    ...
    # FIXED: Parameters passed explicitly from command line/defaults
    generate_realizations(
        ...
        num_realizations=args.num_reals
    )
