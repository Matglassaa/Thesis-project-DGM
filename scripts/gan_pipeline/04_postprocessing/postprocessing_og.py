...
class WellMismatch:
    ...
    Attributes:
        data_name (str): Display name for the baseline dataset in plots.
    ...
    """
    def __init__(self, data_name, gan_name, data_dir, well_data_path):
        """Initializes the WellMismatch evaluator.

        Args:
            data_name (str): Display name for the baseline dataset (e.g., 'Flumy').
    ...
    def plot_vpc(self, num_realizations=10, figsize=None, save_plot=False, output_dir='outputs'):
        ...
        # Based on well loading logic, we evaluate 32 meters
        max_z = 32
        ...
        axes[0].set_title(f'{self.data_name} VPC (Local)', fontsize=14)
        ...
        plt.suptitle(f"Vertical Proportion Curve: {self.data_name} vs. {self.gan_name}", fontsize=16)
        plt.tight_layout()
...
class PostProcessing:
    ...
    def load_data_samples(self, limit=100):
        ...
        """
        print(f"Loading {self.data_name} samples (limit: {limit if limit else 'ALL'})...")
        ...
        print(f"Loaded {len(self.data_samples)} {self.data_name} samples into memory.")
    ...
    def connectivity_and_pattern_analysis(self, facies_list=None, sample_limit=10, recompute_data=False):
        """Executes MPS and Connectivity comparisons for ALL facies using loaded data.
        
        Baseline (Flumy) data is computed once and saved to a CSV. On subsequent runs, 
        ...
        Args:
            ...
            recompute_data (bool, optional): If True, forces a recalculation of the 
                baseline dataset even if a saved CSV exists. Defaults to False.
                
        Returns:
            tuple: (DataFrame of connectivity stats, Data Pattern Counter, GAN Pattern Counter)
        """
        ...
        data_stats_dict = {}
        data_pattern_counter = Counter()
        total_data_patterns = "Loaded from CSV (Not Recomputed)"

        if not recompute_data and os.path.exists(baseline_csv_path):
            print(f"\n--- Loading pre-computed {self.data_name} metrics from: {os.path.basename(baseline_csv_path)} ---")
            df_baseline = pd.read_csv(baseline_csv_path)
            
            for _, row in df_baseline.iterrows():
                data_stats_dict[row['Facies']] = {
                    'Data_Max_Blob': row['Data_Max_Blob'],
                    'Data_Median': row['Data_Median'],
                    'Data_Mean': row['Data_Mean']
                }
        else:
            print(f"\n--- Computing {self.data_name} metrics for {limit_str} samples ---")
            if getattr(self, 'data_samples', None) is None or len(self.data_samples) == 0:
                self.load_data_samples(limit=sample_limit)
            
            data_to_process = self.data_samples[:sample_limit] if sample_limit else self.data_samples
            all_data_blobs = defaultdict(list)
            
            for data in data_to_process:
                patterns, counts = self.get_pattern_counts(data)
                for p, c in zip(patterns, counts):
                    data_pattern_counter[tuple(p)] += c 
                    
                for f_val in facies_list:
                    all_data_blobs[f_val].extend(self.analyze_connectivity(data, f_val))
                    
            total_data_patterns = len(data_pattern_counter)
            
            baseline_rows = []
            for f_val in facies_list:
                d_blobs = all_data_blobs[f_val]
                stats = {
                    'Data_Max_Blob': max(d_blobs) if d_blobs else 0,
                    'Data_Median': np.median(d_blobs) if d_blobs else 0,
                    'Data_Mean': np.mean(d_blobs) if d_blobs else 0
                }
                data_stats_dict[f_val] = stats
            ...
            print(f"Saved {self.data_name} baseline metrics to: {baseline_csv_path}")
        ...
        for f_val in facies_list:
            ...
            d_stats = data_stats_dict.get(f_val, {'Data_Max_Blob': 0, 'Data_Median': 0, 'Data_Mean': 0})
            
            stats_data.append({
                'Facies': f_val,
                'Data_Max_Blob': d_stats['Data_Max_Blob'],
                'GAN_Max_Blob': max(g_blobs) if g_blobs else 0,
                'Data_Median': d_stats['Data_Median'],
                'GAN_Median': np.median(g_blobs) if g_blobs else 0,
                'Data_Mean': d_stats['Data_Mean'],
                'GAN_Mean': np.mean(g_blobs) if g_blobs else 0
            })
            
        df_stats = pd.DataFrame(stats_data)
        df_stats['Data_Mean'] = df_stats['Data_Mean'].round(2)
        ...
        print("="*50)
        print(f"Total unique patterns ({self.data_name}): {total_data_patterns}")
        ...
        return df_stats, data_pattern_counter, gan_pattern_counter
    ...
    def plot_facies_percentages(self, mode='gan', figsize=None, show_plot=True, save_plot=False):
        ...
        if mode == 'both':
            ...
            legend_elements = [
                mpatches.Patch(facecolor='gray', alpha=0.5, edgecolor='black', label=f'{self.data_name} ({len(self.data_samples)} samples)'),
                mpatches.Patch(facecolor='gray', hatch='//', edgecolor='black', label=f'{self.gan_name} ({len(self.gan_data)} samples)')
            ]
            ax.legend(handles=legend_elements, loc='upper right', fontsize=11)
            ax.set_title(f'Facies Volume Distribution: {self.data_name} vs. {self.gan_name}', fontsize=14, pad=15)
            ...
            display_name = self.gan_name if mode == 'gan' else self.data_name
    ...
    def plot_entropy(self, data_source='gan', axis=None, num_slices=9, figsize=None, plot_title=True, show_plot=True, save_plot=False):
        ...
        display_name = self.data_name if data_source == 'real' else self.gan_name
        print(f"\n--- Generating {display_name} Normalized Entropy Matrices ---")
        print(f"{self.data_name} Baseline Proportions: {[round(p, 4) for p in real_global_probs]}")
    ...
    def compare_structural_delentropy(self):
        ...
        if not self.data_samples: self.load_data_samples()
        ...
        print(f"{self.data_name} Mean Delentropy: {real_mean:.4f} bits")
        print(f"{self.gan_name} Mean Delentropy:  {gan_mean:.4f} bits")
        print(f"Difference ({self.gan_name} - {self.data_name}):   {gan_mean - real_mean:.4f} bits")
        
        if abs(gan_mean - real_mean) < 0.1:
            print(f"-> The {self.gan_name} exhibits excellent spatial structural fidelity compared to {self.data_name}.")
        else:
    ...
    def plot_local_delentropy_map(self, data_source='gan', slice_idx=None, window_size=5, figsize=None):
        ...
        if data_source == 'real' and not self.data_samples: self.load_data_samples()
    ...
    def compute_slice_metrics(self, axis='Z', plot=False):
        ...
        with warnings.catch_warnings():
            ...
            with tqdm(total=len(slice_indices), desc=f"JS-entropy 2D ({axis}-axis)") as pbar:
                for slice_idx in slice_indices:
                    ...
                    # Added axis=0 to properly calculate pixel-wise distance
        ...
        print(f"{self.data_name} Entropy: {summary_stats['Real_Entropy_Mean']:.4f} ± {summary_stats['Real_Entropy_Std']:.4f}")
    ...
    def plot_2d_slices(self, data_source='gan', num_samples=1, num_slices=1, axis=None, 
        ...
        if data_source == 'real' and not self.data_samples: 
            self.load_data_samples(limit=num_samples)
        ...
        for idx in range(plot_limit):
            ...
            # When slice_indices overrides num_slices
    ...
    def plot_3d_pyvista(self, data_source='gan', mode='separate', target_filename=None, target_facies=None, 
        ...
        plotter.add_title(title=self.gan_name, fontsize=30, color='b')
...
class DistributionEvaluator:
    ...
    """
    def __init__(self, data_name, gan_name, output_dir, device_type=None):
    ...
    def compute_msswd_mds(self, data_samples, gan_data, random_state=42, save_data=True, load_existing=False):
        ...
        print(f"Preparing tensors for {len(data_samples)} {self.data_name} and {len(gan_data)} {self.gan_name} samples...")
    ...
    def plot_mds_embeddings(self, real_embeddings, gan_embeddings, figsize=None, save_plot=True):
        """Generates a scatter plot of the MDS embeddings.

        Args:
            real_embeddings (numpy.ndarray): 2D array of shape (n_real, 2) with MDS coordinates.
            gan_embeddings (numpy.ndarray): 2D array of shape (n_gan, 2) with MDS coordinates.
            figsize (tuple, optional): Figure size as (width, height) in inches. Defaults to (8, 8).
            save_plot (bool, optional): If True, saves the plot to the output directory. Defaults to True.
        """
        ...
        ax.scatter(
            ...
            s=30, c='#d1d1d1', marker='s', lw=0.5, ec='white', label=f'{self.data_name} Samples'
        )
        ax.scatter(
            gan_embeddings[:, 0], gan_embeddings[:, 1], 
            s=30, c='#003f5c', marker='o', lw=0.5, ec='white', label=f'{self.gan_name} Samples'
        )
        
        ax.set_aspect('equal')
        ax.set_xlabel('Dimension 1', fontsize=12)
        ax.set_ylabel('Dimension 2', fontsize=12)
        ax.set_title(f'MS-SWD Multidimensional Scaling: {self.data_name} vs. {self.gan_name}', fontsize=14, pad=15)
